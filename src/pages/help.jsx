    ...
    <div style={{
      ...
      {open && (
        <div style={{ padding: "0 16px 16px", borderTop: "1px solid var(--border)" }}>
          ...
          <div style={{ marginBottom: note ? "10px" : "0" }}>
            <div style={{
              fontSize: "11px",
              fontWeight: "600",
              textTransform: "uppercase",
            ...
            <Code language="bash" theme="dark">{fix}</Code>
          </div>
...
function IssueCategory({ category, items }) {
  return (
    <div style={{ marginBottom: "40px" }}>
      <h2 style={{ marginBottom: "16px", fontSize: "18px" }}>{category}</h2>
      {items.map((item, i) => (
    ...
    : issues
        .map(cat => ({
          ...
          items: cat.items.filter(item =>
            ...
            item.fix.toLowerCase().includes(search.toLowerCase())
          )
        }))
  ...
  return (
    <div className={s.page}>
      <div className={s.pageHeader}>
        <span className={s.pageTag}>TROUBLESHOOTING</span>
        <h1>🐛 Common Errors & Fixes</h1>
        <p>Every error you're likely to hit, with exact fixes. Built from real install sessions.</p>
      </div>

      <div className={`${s.callout} ${s.calloutInfo}`} style={{ marginBottom: "28px" }}>
        <span className={s.calloutIcon}>💡</span>
        <div className={s.calloutBody}>
          <strong>Before anything else — check this order:</strong>
          <ol style={{ margin: "8px 0 0 0", paddingLeft: "20px", lineHeight: "2" }}>
            <li>Extension loaded in browser ✓</li>
            <li>Extension ID registered with <code>scrapper-register-ext</code> ✓</li>
            <li><code>scrapper-start</code> running in terminal ✓</li>
            <li>Actually browsed a site with the extension active ✓</li>
          </ol>
          <div style={{ marginTop: "8px", fontSize: "13px", color: "var(--text3)" }}>
            90% of issues come from the wrong order or a stale extension ID.
          </div>
        </div>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        filtered.map((cat, i) => (
          <IssueCategory key={i} category={cat.category} items={cat.items} />
        ))
      )}

      <div className={`${s.callout} ${s.calloutWarning}`} style={{ marginTop: "16px" }}>
        <span className={s.calloutIcon}>🐛</span>
        <div className={s.calloutBody}>
          Hit an error not listed here?{" "}
          <a 
            href="https://github.com/BunElysiaReact/SCRAPY/issues"
            target="_blank"
            rel="noreferrer"
          ...
          with your error message and OS — we will fix it and add it to this page.
        </div>
      </div>

      <div className={s.btnRow}>
        <Link to="/faq" className={s.btnPrimary}>❓ FAQ →</Link>
        <Link to="/installation/linux" className={s.btnSecondary}>⚡ Back to Install</Link>
      </div>
    </div>
  );
}